...
object AstMacro:
  ...
  private def astImpl(stringContextExpr: Expr[StringContext],
                      argsExpr: Expr[Seq[Any]]
                     )(using quotes: Quotes) : Expr[Ast[Int]] = {
    val stringContext = stringContextExpr.valueOrAbort

    stringContext.asSingleStringOrFail match {
      case Some(s) => GrammarParser.parseAll(s) match
  ...
  private def evalImpl(stringContextExpr: Expr[StringContext],
                      argsExpr: Expr[Seq[Any]]
    ...
    stringContext.asSingleStringOrFail match {
      case Some(s) => GrammarParser.parseAll(s) match
        case Some(ast) => ast.evaluate match
