...
object ZonedDateTimeMacro:
  ...
  private def dateImpl(contextExpr: Expr[StringContext], argsExpr: Expr[Seq[Any]])(using q: Quotes): Expr[ZonedDateTime] = {
    val context = contextExpr.valueOrAbort

    context.parts.toList match
      case s :: Nil => parse(s.trim()) match
        case Left(message) => fail(message, contextExpr)
  ...
  def errorMessage(s: String, e: Throwable) : String = {
    val position = Some(e).collect {
      case ex : DateTimeParseException => ex.getErrorIndex()
    }
